...
BEGIN
  ...
  SET @Actual_Values +=
    CASE 
      ...
      WHEN @Data_Type COLLATE DATABASE_DEFAULT IN ('ntext','xml')                                                           THEN 'COALESCE(''''''''  + REPLACE(CONVERT(nvarchar(max),' + @Column_Name + '),'''''''','''''''''''')+'''''''',''NULL'')'
      ...
      WHEN @Data_Type COLLATE DATABASE_DEFAULT IN ('sql_variant')                                                           THEN 'CASE WHEN SQL_VARIANT_PROPERTY(' + @Column_Name + ',''BaseType'') IN (''varchar'',''nvarchar'')                                                          THEN COALESCE(''N'''''' + REPLACE(CAST(' + @Column_Name + ' AS NVARCHAR(MAX)),'''''''','''''''''''')+'''''''',''NULL'') 
                                                                                                                                       WHEN SQL_VARIANT_PROPERTY(' + @Column_Name + ',''BaseType'') IN (''char'',''nchar'')                                                                THEN COALESCE(''N'''''' + REPLACE(RTRIM(CAST(' + @Column_Name + ' AS NVARCHAR(MAX))),'''''''','''''''''''')+'''''''',''NULL'')
                                                                                                                                       WHEN SQL_VARIANT_PROPERTY(' + @Column_Name + ',''BaseType'') IN (''datetime'',''smalldatetime'',''datetime2'',''date'',''datetimeoffset'',''time'') THEN COALESCE(''''''''  + RTRIM(CONVERT(char,' + @Column_Name + ',127))+'''''''',''NULL'')
                                                                                                                                       WHEN SQL_VARIANT_PROPERTY(' + @Column_Name + ',''BaseType'') IN (''uniqueidentifier'')                                                              THEN COALESCE(''N'''''' + REPLACE(CONVERT(char(36),RTRIM(CAST(' + @Column_Name + ' AS NVARCHAR(MAX)))),'''''''','''''''''''')+'''''''',''NULL'')
                                                                                                                                       WHEN SQL_VARIANT_PROPERTY(' + @Column_Name + ',''BaseType'') IN (''float'',''real'',''money'',''smallmoney'')                                       THEN COALESCE(LTRIM(RTRIM(' + 'CONVERT(char, ' + @Column_Name + ',2)' + ')),''NULL'')
