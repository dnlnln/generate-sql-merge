...
BEGIN
  ...
  SET @sql =
    ...
    FROM ' + COALESCE(PARSENAME(@target_table COLLATE DATABASE_DEFAULT,3),QUOTENAME(DB_NAME())) + '.sys.columns c (NOLOCK)
    INNER JOIN ' + COALESCE(PARSENAME(@target_table COLLATE DATABASE_DEFAULT,3),QUOTENAME(DB_NAME())) + '.sys.tables t (NOLOCK) ON c.object_id = t.object_id
    WHERE t.name = ''' + PARSENAME(@target_table COLLATE DATABASE_DEFAULT,1) + '''' +
    ' AND t.schema_id = ' + '''' + COALESCE(SCHEMA_ID(PARSENAME(@target_table COLLATE DATABASE_DEFAULT,2)), SCHEMA_ID(@schema COLLATE DATABASE_DEFAULT), SCHEMA_ID()) + '''' + ' AND c.[name] = ''' + @hash_compare_column COLLATE DATABASE_DEFAULT + ''''
...
BEGIN
  ...
  JOIN sys.types bt ON tp.system_type_id = bt.user_type_id AND tp.system_type_id = bt.system_type_id
  ...
  (
    ...
    WHERE pk.type = 'PK'
      AND OBJECT_NAME(pk.parent_object_id) = @Internal_Table_Name COLLATE DATABASE_DEFAULT
      AND pk.schema_id = COALESCE(SCHEMA_ID(@schema COLLATE DATABASE_DEFAULT), SCHEMA_ID())
...
BEGIN
  ...
  JOIN sys.columns c ON ic.object_id = c.object_id AND ic.column_id = c.column_id
  WHERE kc.type = 'PK'
    AND OBJECT_NAME(kc.parent_object_id) = @Internal_Table_Name COLLATE DATABASE_DEFAULT
    AND OBJECT_SCHEMA_NAME(kc.parent_object_id) = COALESCE(@schema COLLATE DATABASE_DEFAULT, SCHEMA_NAME())
