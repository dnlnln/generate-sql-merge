...
BEGIN
  ...
  SET @Actual_Values +=
    'COALESCE(' + (CASE 
      ...
      WHEN @Data_Type COLLATE DATABASE_DEFAULT IN ('sql_variant')                                                           THEN 'CASE WHEN SQL_VARIANT_PROPERTY(' + @Column_Name + ',''BaseType'') IN (''varchar'',''nvarchar'')                                                          THEN ''N'''''' + REPLACE(CAST(' + @Column_Name + ' AS NVARCHAR(MAX)),'''''''','''''''''''')+'''''''' 
